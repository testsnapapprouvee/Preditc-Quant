...
def calculate_metrics_legacy(series):
    ...
    if len(returns) == 0:
        return {k: 0 for k in ['Cumul', 'CAGR', 'MaxDD', 'Vol', 'Sharpe', 'Calmar', 'Sortino', 'Omega', 'VaR_95', 'CVaR_95', 'Skew', 'Kurt']}
...
def create_optimization_objective(backtest_func, calculate_metrics_func, 
    ...
    def objective(params):
        ...
        try:
            ...
            if profile == 'PERFORMANCE ONLY':
                # Score = CAGR brute, ignore risk
                score = metrics['CAGR']
            else:
                # Score multi-objective existant
            ...
            # Differential evolution cherche à minimiser → retourner négatif
            return -score
...
with st.sidebar:
    ...
    alloc_prud = st.slider("Prudent Mode (X1%)", 0, 100, alloc_prud_default, 10, help="% allocated to safe asset in Prudence regime")
    alloc_crash = st.slider("Crisis Mode (X1%)", 0, 100, alloc_crash_default, 10, help="% allocated to safe asset in Crisis regime")
